...
// a timer to check if 30 seconds passed(every 30 seconds to multiplie each ghost)
...
void detectGhost()
{
    for(int i = 0; i < loopCount; i++) { // Check pacmans position against all of the ghosts
 ...
 */
void idle()
// if ghosts grows to 32, stop the game.
{  if(ghostAmount < 32){
    if(!paused) { // If the game is not paused
        switch (stateGame) { // Check what state the game is in
            ...
            case PLAY:
                ...
                detectGhost();
                for (int i = 0; i < loopCount; i++) { // Move the ghosts in the maze
    ...
    if(time(NULL) - first > 30){
    	
    	ifGrows = true;
    	
    }
    // if so, multiplie each live ghost, then re-timing.
    if(ifGrows){
    	int i;
    	int count = 0;
    	if(ghostAmount < loopCount){
    		for(i = 0; i < ghostAmount; i++){
    		
    			ghosts[i + ghostAmount] = Ghost(13.5f, 19.0f, ghosts[i].ghostColour);
    			count++;
    		}}
    	ghostAmount = ghostAmount + count;
    	ifGrows = false;
    	first = time(NULL);
    }
