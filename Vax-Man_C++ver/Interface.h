...
extern int score;
extern int lives;
extern int ghostAmount;


void drawGhosts(int ghostAmount)
{  
    glPushMatrix();
    translateBottomLeft();
    translateToMazeCoords(15.0, 32.5); // Translate to the correct area of the screen ready for drawing
    translateToMazeCoords(10, -1.0); // Translate to below "SCORE" ready to draw numbers
    std::string str = std::to_string(ghostAmount); // Convert the current score to a string to enable iteration through number
